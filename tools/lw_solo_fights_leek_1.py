...
class LeekWarsAutoFighter:
    ...
    def run_solo_fights(self, num_fights=None, quick_mode=False):
        ...
        # Use only the first leek
        if leek_list:
            leek_list = [leek_list[0]]
            print(f"\n📋 Using only the first leek:")
            print(f"   1. {leek_list[0]['name']} (Level {leek_list[0]['level']})")
...
def main():
    ...
    try:
        ...
        if fighter.total_fights > 0:
            ...
            # Run only 15 fights with the first leek
            fights_to_run = min(15, fighter.total_fights)
